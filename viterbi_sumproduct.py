...
def sum_log(a, axis=0):
    ...
    :param axis: axis along which the sum is performed, useful if 'a' is a matrix
...
def forward(A, b, E, mode):
    '''
    Forward pass: computes the logarithms of alpha-messages associated to the Sum-Product
    ...
    :param mode: string to precise how to compute the log alpha-messages. Must be 'max' for Viterbi,
            'sum' for Sum-Product
    :return: if 'max', matrix of log alpha-messages and the argmax matrix ; if 'sum', matrix of
            log alpha-messages
    '''
...
def backward(A, E):
    '''
    Backward pass: computes the logarithms of beta-messages for the Sum-Product algorithm
    :param A: matrix of state-transition probabilities (M rows, M columns)
    :param E: matrix of emission probabilities computed with Felstenstein algorithm (M rows, L columns)
    ...
    for t in range(L-2, -1, -1):
        beta_log[:, t] = sum_log(np.log(A) + np.log(E[:, t+1]) + beta_log[:, t+1], axis=1)
    return beta_log
...
def sum_product(A, b, E):
    ...
    # Forward and backward procedure to compute the logarithms of alpha and beta messages
